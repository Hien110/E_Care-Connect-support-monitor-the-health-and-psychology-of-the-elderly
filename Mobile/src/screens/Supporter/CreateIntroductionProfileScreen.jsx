...
const CreateIntroductionProfile = ({ navigation }) => {
  ...
  const [selectedShifts, setSelectedShifts] = useState(DAYS.reduce((acc, d) => ({ ...acc, [d]: [] }), {}));
  ...
  const disableCreate = useMemo(() => {
    const hasAnyShift = Object.values(selectedShifts).some((arr) => Array.isArray(arr) && arr.length > 0);
  ...
  const toggleTempShift = (value) =>
    setTempSelections((prev) => (prev.includes(value) ? prev.filter((v) => v !== value) : [...prev, value]));
  ...
  const buildSchedulePayload = () => {
    ...
    for (const day of DAYS) {
      ...
      slots.forEach((slot) => out.push({ dayOfWeek: DAY_TO_NUM[day], timeSlots: slot }));
    }
    return out;
  };
  ...
  const onCreateProfile = async () => {
    ...
    if (!jobDescription || jobDescription.trim().length < 500) {
      Alert.alert('Mô tả quá ngắn', 'Vui lòng nhập mô tả công việc ít nhất 500 ký tự.');
      return;
    }
    ...
    const payload = {
      experience: { description: jobDescription.trim(), totalYears: Math.min(Math.max(Number(experienceYears), 0), 60) },
      schedule,
        ...
        <View style={styles.dropdownContainer}>
          ...
          <FlatList
            ...
            renderItem={({ item }) => {
              ...
              return (
                <TouchableOpacity style={styles.checkRow} onPress={() => toggleTempShift(item.value)} activeOpacity={0.85}>
    ...
    <Modal visible={showSuccess} transparent animationType="fade" onRequestClose={() => setShowSuccess(false)}>
      <View style={styles.successOverlay}>
        <View style={styles.successCard}>
          ...
          <Text style={styles.successSub}>Hồ sơ của bạn đã được tạo thành công. Bây giờ khách hàng có thể tìm thấy và liên hệ với bạn.</Text>
          ...
          <TouchableOpacity style={styles.primaryBtn} onPress={() => { setShowSuccess(false); navigation?.navigate?.('MySupporterProfile'); }}>
            <Text style={styles.primaryBtnText}>Xem hồ sơ</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.ghostBtn} onPress={() => { setShowSuccess(false); navigation?.navigate?.('EditSupporterProfile'); }}>
  ...
  // Chuỗi ca ngắn gọn hiển thị trong card ngày
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.profileIconSection}>
          <View style={styles.profileIcon}><MaterialIcons name="person" size={32} color="#ffffff" /></View>
        </View>

        <View style={styles.titleSection}>
          <Text style={styles.mainTitle}>Tạo hồ sơ Supporter</Text>
          <Text style={styles.subtitle}>Hãy tạo hồ sơ của bạn để trở thành một Supporter. Bạn sẽ được hưởng quyền lợi từ chúng tôi.</Text>
        </View>

        <View style={styles.benefitsSection}>
          <Text style={styles.sectionTitle}>Lợi ích khi có hồ sơ</Text>
          <View style={styles.benefitItem}><View style={styles.checkIcon}><Icon name="checkmark" size={16} color="#ffffff" /></View><Text style={styles.benefitText}>Ưu tiên các việc làm hấp dẫn</Text></View>
          <View style={styles.benefitItem}><View style={styles.checkIcon}><Icon name="checkmark" size={16} color="#ffffff" /></View><Text style={styles.benefitText}>Hỗ trợ được ưu tiên</Text></View>
          <View style={styles.benefitItem}><View style={styles.checkIcon}><Icon name="checkmark" size={16} color="#ffffff" /></View><Text style={styles.benefitText}>Nhận thêm nhiều công việc</Text></View>
        </View>
        ...
        <View style={styles.formSection}>
          ...
          <TextInput
            ...
            maxLength={2}
          />
          {!!experienceYearsError && <Text style={{ color: '#EF4444', marginTop: 6 }}>{experienceYearsError}</Text>}
        </View>
        ...
        <TouchableOpacity style={[styles.createButton, disableCreate && { opacity: 0.6 }]} onPress={onCreateProfile} disabled={disableCreate}>
...
const styles = StyleSheet.create({
  ...
  checkIcon: { width: 20, height: 20, borderRadius: 10, backgroundColor: '#4A90E2', alignItems: 'center', justifyContent: 'center', marginRight: 12 },
  ...
  /* === NEW: Card cho từng ngày có border/bo góc như ảnh === */
  dayCard: {
    flexDirection: 'row',
    alignItems: 'center',
    ...
    borderWidth: 1,
    borderColor: '#E3E8EF',     // viền nhẹ đúng style
    backgroundColor: '#FFFFFF',
    borderRadius: 12,            // bo góc
    marginBottom: 10,
  },
  dayLeft: { flex: 1 },
  dayTitle: { fontSize: 16, color: '#0B1220', fontWeight: '700', marginBottom: 2 },
  daySubtitle: { fontSize: 14, color: '#667085' }, // “Chọn ca làm việc” / ca đã chọn
  ...
  dropdownContainer: { backgroundColor: '#ffffff', borderRadius: 16, padding: 16, width: '100%', maxWidth: 360 },
  ...
  btnGhost: { paddingVertical: 10, paddingHorizontal: 16, borderRadius: 10, borderWidth: 1, borderColor: '#DFE8FF', marginRight: 10, backgroundColor: '#fff' },
